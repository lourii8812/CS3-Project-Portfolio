# WORKSHEET 1

## Observe and Analyze a Real-World Process

---

## 1. Project Information

| Information                | Details |
| -------------------------- | ------- |
| **Proposed Project Title** | Attendance Checker |
| **Team Members**           | 1. Lourence B. Villegas |
|                            | 2. Kobe Karl M. Sasuman |
|                            | 3. Lienra Viem M. Chua  |
|                            | 4. Suegene Rei S. Yaranon |
|                            | 5. Audrey Bless E. Quiapo |
|                            | 6. Mohammad Madki |
| **PSHS-DRC Area/Location** | Classrooms |

---

## 2. Observe the Current Process

### 1. What process did you observe?

> *The daily classroom student attendance logging and monitoring process where students type their information manually.*

<br>

### 2. What is the purpose of this process?

> *To track who is present or late every day, make sure late students get an admission slip, and help teachers keep clear attendance records.*

<br>

### 3. Who are involved in the process?

**(Up to two roles only.)**

| Person/Role | Responsibility in the Process |
| ----------- | ----------------------------- |
| 1. Student  | Honestly input their name and time they arrived |
| 2. Teacher  | Check and monitor times of arrival |

### 4. What are the steps in the current process?

| Step | What happens? | Person/Role Involved |
| ---- | ------------- | -------------------- |
| 1 Student arrives at classroom | Student checks in | Student |
| 2 Program use | Student inputs the data | Student |
| 3 Program checks if late | If input time > 7:40 AM = Late | N\A (Program) |
| 4 If student is late | Program tells student to get an admission slip and marks the student as late | N\A (Program) |
| 5 If student is not late | Program marks Student as present | N\A (Program)|
| 6 After 4:30, check if all students are present | Teacher tells program to display student name, status, and time of arrival | Teacher | 
---

## 3. Analyze the Problem

### 1. What problems or difficulties did you observe?

> *Tracking the present students and monitoring their time of arrival is a hassle towards the teacher in charge (adviser).*

<br>

### 2. Which part of the process could be improved or automated?

> *The process of tracking attendance and monitoring time of arrival.*

<br>

---

## 4. Propose a Simple Solution

### 1. What would your proposed system do?

> The system will let students quickly login when they arrive at the classroom and save their check-in time automatically. It will mark anyone arriving after 7:40 as late, remind them to get an admission slip, and make a clean daily attendance report for the teacher to check at the end of the day.

<br>

### 2. Who would use the system?

| User/Role | Possible Use |
| --------- | ------------ |
| 1.*Role 1* | *How will they use the program* |
| 2.*Role 2* | *How will they use the program* |

### 3. What are the essential features?

> **Keep your proposed system simple. Your project will be subject to teacher approval.**

| Feature | Purpose |
| ------- | ------- |
| 1. Student Check In     |  To allow students to input their name and record their arrival time       |
| 2. Automatic Attendance Status      | To automatically mark the students Present or late, basing off the set time 7:40 A        |
| 3. Attendance Records     | To allow the teacher to view the student's name, attendance status, and time of arrival       |
